...
export function* calculateNextMove() {
    ...
    if (nextMove) {
        yield put({type: UPDATE_BOX, id: nextMove - 1, value: gameContext.computersTeam})
    }
}
...
function* threeInARow(boxes, playerSymbol) {
    ...
    for (const wc of winningCombinations) {
        if (playersBoxes.includes(wc[0]) && playersBoxes.includes(wc[1]) && playersBoxes.includes(wc[2])) {
            return {
                winner: playerSymbol,
...
function* emptySide(boxes) {
    ...
    for (const side of sides) {
        if (emptyBoxes.includes(side[0]) && emptyBoxes.includes(side[1]) && emptyBoxes.includes(side[2])) {
            return side[0]
        }
    }
}
